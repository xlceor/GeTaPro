...
import { FaBold, FaItalic, FaHeading } from 'react-icons/fa'; // Importar los iconos desde React Icons
...
export default function Modal({ content, setContent, onSave }: UploadProps) {
  ...
  React.useEffect(() => {
    if (editor) {
      console.log('Editor listo:', editor.getJSON());
    }
  }, [editor]);
      ...
      <div className="h-full">
        ...
        <div className="flex w-full justify-center">
          <div className="flex w-2/3 justify-center bg-gray-200 p-2 space-x-4 mb-2">
            <button
              className="p-3 flex  justify-center items-center bg-gray-300 w-1/6 rounded-md text-gray-700 hover:bg-gray-300"
              onClick={() => editor?.chain().focus().toggleBold().run()}
            >
              <FaBold /> {/* Icono de negrita */}
            </button>
            <button
              className="p-3 flex  justify-center items-center bg-gray-300 w-1/6 rounded-md text-gray-700 hover:bg-gray-300"
              onClick={() => editor?.chain().focus().toggleItalic().run()}
            >
              <FaItalic /> {/* Icono de cursiva */}
            </button>
            <button
              className="p-3 flex  justify-center items-center bg-gray-300 w-1/6 rounded-md text-gray-700 hover:bg-gray-300"
              onClick={() => editor?.chain().focus().toggleHeading({ level: 1 }).run()}
            >
              <FaHeading /> {/* Icono de título */}
            </button>
            <button
              className="p-3 flex  justify-center items-center bg-gray-300 w-1/6 rounded-md text-gray-700 hover:bg-gray-300"
        ...
        <div className="p-4 bg-white rounded-xl outline-none shadow-sm h-full">
          <EditorContent
            editor={editor}
            className="h-full w-full outline-none prose prose-lg"
          />
        </div>
      </div>
      <style jsx global>{`
